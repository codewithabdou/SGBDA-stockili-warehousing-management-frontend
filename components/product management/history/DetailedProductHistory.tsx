...
const DetailedProductHistory = ({ productId }: { productId: string }) => {
  ...
  useEffect(() => {
    const fetchProductHistory = async () => {
      const history = await getDetailedHistory({ productId });
                ...
                (
                  ...
                  <li key={index} className="mb-10 ms-6">
                    ...
                    <h3 className="flex items-center mb-1 text-lg font-semibold text-gray-900 dark:text-white">
                      ...
                      {historyItem.type === "in" ? "in" : "from"}{" "}
                      {historyItem.zone.name}
                      {!index && (
...
export default DetailedProductHistory;
